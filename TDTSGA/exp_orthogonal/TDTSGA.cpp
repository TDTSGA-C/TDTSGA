...
double runTDTSGA(string XmlFile, string RscAlcFile, Orthogonal& orthogonal, double& SchTime, int& iteration){
    clock_t start = clock();
    ReadFile(XmlFile, RscAlcFile);         //read model information  XmlFile, RscAlcFile -xy4
    ConfigParameter_TSTDGA(orthogonal);    //set the parameter values
    ...
    while ((double) (clock() - start) / CLOCKS_PER_SEC < SchTime){
        ...
        for (int n = 0; n < Parameter_TDTSGA.NumOfChormPerPop; n += 2) {
            ...
            if (stg == 1) {
                ...
                Mtn(NewPopulation[n], Parameter_TDTSGA.MutationStg1Rate, stg);
                Mtn(NewPopulation[n+1], Parameter_TDTSGA.MutationStg1Rate, stg);
            } else {
                CrsTS( chrom1,chrom2);
                NewPopulation[n] = chrom1;
                NewPopulation[n+1] = chrom2;
                Mtn(NewPopulation[n], Parameter_TDTSGA.MutationStg2Rate, stg);
                Mtn(NewPopulation[n+1], Parameter_TDTSGA.MutationStg2Rate, stg);
            }
        }
