...
void ReadFile(string XmlFile, string RscAlcFile) {
    ...
    ParChildTranFileSizeSum.resize(comConst.NumOfTsk, vector<double>(comConst.NumOfTsk, 0));
...
void ConfigParameter_TSTDGA(Orthogonal orthogonal) {
    Parameter_TDTSGA.NumOfChormPerPop = Tasks.size() * orthogonal.NumOfChormPerPopRate;
    if (Parameter_TDTSGA.NumOfChormPerPop % 2 == 1) {
        ++Parameter_TDTSGA.NumOfChormPerPop;
    }
    Parameter_TDTSGA.beta = orthogonal.beta;
    Parameter_TDTSGA.MutationStg1Rate = orthogonal.Mtn1;
    Parameter_TDTSGA.MutationStg2Rate = orthogonal.Mtn2;
    Parameter_TDTSGA.RunTimeRatioOfStg1 = orthogonal.RunTimeRatioOfStg1Rate;
}